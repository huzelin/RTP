/*!
 * \file grpc_bad_node_detector.cc
...
GrpcBadNodeDetector::GrpcBadNodeDetector(NodeCollectionInterface* node_collection,
                                         GrpcChannelPool* channel_pool) :
    BadNodeDetector(node_collection),
    channel_pool_(channel_pool),
    running_(false) { }

GrpcBadNodeDetector::~GrpcBadNodeDetector() {
  StopDetect();
}

void GrpcBadNodeDetector::StartDetect() {
  if (running_) {
...
void GrpcBadNodeDetector::StopDetect() {
  running_.Stop();
...
void GrpcBadNodeDetector::DoDetect(std::set<std::string>& specList) {
