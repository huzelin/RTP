...
# Usage:
#   rtp_list_unique(<list_variable> [<list_variable>] [...])
macro(rtp_list_unique)
  foreach(__lst ${ARGN})
    if(${__lst})
...
# Usage:
#   rtp_clear_vars(<variables_list>)
macro(rtp_clear_vars)
  foreach(_var ${ARGN})
    unset(${_var})
  endforeach()
endmacro()
...
# Usage:
#   rtp_string_unique(<string_variable>)
function(rtp_string_unique __string)
  if(${__string})
    set(__list ${${__string}})
...
# Usage:
#   rtp_merge_flag_lists(out_variable <list1> [<list2>] [<list3>] ...)
function(rtp_merge_flag_lists out_var)
  set(__result "")
  foreach(__list ${ARGN})
...
# Usage:
#   rtp_convert_absolute_paths(<list_variable>)
function(rtp_convert_absolute_paths variable)
  set(__dlist "")
...
# Usage:
#   rtp_parse_header(<file> <define1> <define2> <define3> ..)
macro(rtp_parse_header FILENAME FILE_VAR)
  set(vars_regex "")
...
# Usage:
#   rtp_parse_header_single_define(<library_name> <file> <define_name>)
function(rtp_parse_header_single_define LIBNAME HDR_PATH VARNAME)
  set(${LIBNAME}_H "")
  if(EXISTS "${HDR_PATH}")
...
# Usage:
#   rtp_option(<option_variable> "doc string" <initial value or boolean expression> [IF <condition>])
function(rtp_option variable description value)
  set(__value ${value})
  set(__condition "")
...
# Usage:
#   rtp_compare_lists(<list_variable> <list2_variable> [description])
function(rtp_compare_lists list1 list2 desc)
  set(__list1 ${${list1}})
  set(__list2 ${${list2}})
  list(SORT __list1)
  list(SORT __list2)
...
# Usage:
#   rtp_warnings_disable(<CMAKE_[C|CXX]_FLAGS[_CONFIGURATION]> -Wshadow /wd4996 ..,)
macro(rtp_warnings_disable)
  set(_flag_vars "")
  set(_msvc_warnings "")
  set(_gxx_warnings "")
  ...
  rtp_clear_vars(_flag_vars _msvc_warnings _gxx_warnings)
endmacro()

################################################################################################
# Helper function get current definitions
# Usage:
#   rtp_get_current_definitions(<definitions_variable>)
function(rtp_get_current_definitions definitions_var)
...
# Usage:
#   rtp_get_current_cflags(<cflagslist_variable>)
function(rtp_get_current_cflags cflags_var)
  get_property(current_includes DIRECTORY PROPERTY INCLUDE_DIRECTORIES)
  rtp_convert_absolute_paths(current_includes)
  rtp_get_current_definitions(cflags)

  foreach(i ${current_includes})
    list(APPEND cflags "-I${i}")
  endforeach()

  rtp_list_unique(cflags)
...
# Usage:
#   rtp_parse_linker_libs(<rtp_LINKER_LIBS_var> <directories_var> <libflags_var> <frameworks_var>)
function(rtp_parse_linker_libs rtp_LINKER_LIBS_variable folders_var flags_var frameworks_var)
  ...
  foreach(list_elem ${${rtp_LINKER_LIBS_variable}})
  ...
  rtp_list_unique(libflags folders)
...
# Usage:
#   rtp_detect_darwin_version(<version_variable>)
function(rtp_detect_darwin_version output_var)
  if(APPLE)
...
function(rtp_source_group group)
...
function(rtp_add_test source_cxx prefix)
  if(NOT ${source_cxx})
      list(APPEND SOURCE_ ${source_cxx})
      foreach(source ${SOURCE_})
          string(REGEX REPLACE ".*/${prefix}/" "" pname ${source})
